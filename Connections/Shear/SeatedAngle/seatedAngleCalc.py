...
def finConn(uiObj):
            ...
            if pitch >= min_pitch:
                ...
                if bolts_required % 2 == 0 or bolts_required % 2 != 0:
                    for k in range(0, K):
                        M2 = M2 + 2 * (
                            bolt_shear_capacity * (
                            (length_avail / 2 - k * pitch) ** 2 / (length_avail / 2 - k * pitch)));
    ...
    if web_plate_l != 0:
        ...
        moment_capacity = 1.2 * (web_plate_fy / 1.1) * (
            web_plate_t * boltParameters['plateheightoptional'] * boltParameters['plateheightoptional']) / 6 * 0.001;
        moment_capacity = round(moment_capacity * 0.001, 3);
    ...
    if web_plate_l == 0:
        ...
        if min_plate_thk > web_plate_t:  # Change
            ...
            if boltParameters['numofcol'] == 1:
                new_bolt_param['pitch'] = (web_plate_l_opt - 2 * boltParameters['enddist']) / (
                    boltParameters['numofrow'] - 1)
    ...
    if web_plate_l != 0:
        if web_plate_t < min_plate_thk:
            ...
            logger.error(
                ": Plate thickness provided is less than the minimum required [Ref. INSDAG detailing manual, 2002]")
    ...
    if boltParameters['numofcol'] == 1:
        web_plate_l_req2 = (boltParameters['numofbolts'] - 1) * boltParameters['minpitch'] + 2 * boltParameters[
            'enddist'];
    ...
    # Multi line of bolts
    if boltParameters['numofcol'] == 2:
        web_plate_l_req2 = (boltParameters['numofrow'] - 1) * boltParameters['minpitch'] + 2 * boltParameters[
            'enddist'];
    ...
    # Delete dictionary for unsafe design
    else:
        if weld_t_req > weld_t:
            for k in outputObj.keys():
                for key in outputObj[k].keys():
                    outputObj[k][key] = ""
