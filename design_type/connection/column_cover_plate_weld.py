...
class ColumnCoverPlateWeld(MomentConnection):
    ...
    #
        ...
        def warn_text(self):
            ...
            """
            Function to give logger warning when any old value is selected from Columns and columns table.
            """
        ...
        def hard_values(self):
            ...
            # refer Osi file
            # load
            self.load.axial_force = 0 # KN
            self.load.shear_force = 0 # KN
            self.load.moment = 0 # KNM
            self.section.fy = 0
            self.section.fu = 0
            #  Flange Weld
            self.flange_weld.size =0 # mm
            # self.flangespace = 15 #mm
            self.flange_weld.length =0
            self.flange_weld.height = 0
            #  Flange plate
            self.flange_plate.thickness_provided = 0
            self.flange_plate.height = 0
            self.flange_plate.length = 0
            #  Web Weld
            self.web_weld.size = 0 # mm
            # self.webspace = 15  # mm
            self.web_weld.length = 0
            self.web_weld.height = 0
            #  Web plate
            self.web_plate.thickness_provided =0
            self.web_plate.length = 0
            self.web_plate.height = 0
            #  Inner Flange weld
            self.flange_weld.size = 0  # mm
            self.flange_plate.thickness_provided = 0
        ...
        def member_capacity(self):
            ...
            design_shear_capacity = ((self.section.depth - (
            ...
            self.section.plastic_moment_capacty(beta_b=beta_b, Z_p=self.Z_p,
            ...
            #     pass
            self.moment_web = (self.Z_w / (
                self.section.plast_sec_mod_z)) * self.load.moment  # KNmm todo add in ddcl # z_w of web & z_p  of section
            print('plast_sec_mod_z', self.section.plast_sec_mod_z)
            self.moment_flange = ((self.load.moment) - self.moment_web)  # KNmm todo add in ddcl

            if self.load.moment > self.section.moment_capacity / 1000000:  # KNmm
            ...
            if self.section.tension_capacity_web < self.axial_force_w:
                ...
                self.section.tension_capacity_flange = min(self.tension_yielding_capacity_flange,
                                                           self.tension_rupture_capacity_flange)
                ...
                if self.section.tension_capacity_flange > self.flange_force:
                    ...
                    if len(self.flange_plate_thickness_possible) == 0 or self.web_plate_thickness_possible == 0:
                        ...
                        print("tension__capacity of flange", self.section.tension_capacity_flange)
            ...
            if self.design_status == True:
                ...
                logger.error(
                    ":tension capacity of section is less than applied loads, Please select larger sections or decrease loads")
        ...
        def web_plate_weld(self):
            ...
            while self.design_status == False:
                ...
                self.l_req_weblength = round_up(
                    ((2 * self.available_long_web_length) + self.web_plate.height + (2 * self.web_weld.size)), 5)
                self.web_weld.get_weld_stress(weld_shear=self.load.shear_force * 1000, weld_axial=self.axial_force_w,
                                              weld_twist=self.weld_twist, Ip_weld=Ip_weld, y_max=y_max, x_max=x_max,
            ...
            if self.preference == "Outside":
                ...
                if self.design_status == True:
                    ...
                    self.flange_weld.length = round_up(
                        ((2 * self.available_long_flange_length) + self.flange_plate.gap), 5)
                ...
                if self.total_height_of_inner_plate > 0:
                    ...
                    self.flange_weld.Innerheight = round_down(
                        (self.flange_plate.Innerheight - 2 * self.flange_weld.size), 5)
                    if self.flange_weld.Innerheight <= 0:
                        self.design_status = False
                        logger.error(
                            " : asaaa Inner plate is not possible, select preference outside")
                    else:
                        ...
                        while self.design_status == False:
                            ...
                            self.l_req_innerflangelength = (
                                                                   ...
                                                                   2 * self.flange_weld.size)
                            self.flange_weld.stress = self.flange_force / self.l_req_innerflangelength
                            if self.flange_weld.stress < self.flange_weld.strength:
                    ...
                    # self.available_long_innerflange_length = self.available_long_flange_length # half of the one side of the flange plate
                    print("self.available_long_length", self.available_long_flange_length)
                else:
                    self.flange_plate.Innerheight = 0
                    self.flange_weld.Innerheight = 0
                ...
                # print("length", self.available_long_innerflange_length)
                if self.design_status == True:
                    # Outer Plate Details
                    self.flange_weld.length = round_up(
                        ((2 * self.available_long_flange_length) + self.flange_plate.gap), 5)
                    ...
                    self.flange_weld.height = round_down((self.flange_plate.height - (2 * self.flange_weld.size)), 5)
                    # Inner Plate Details
                    self.flange_weld.Innerlength = round_up(
                        ((2 * self.available_long_innerflange_length) + self.flange_plate.gap), 5)
                    self.flange_plate.Innerlength = round_up(
                        (self.flange_weld.Innerlength + (2 * self.flange_weld.size)), 5)
                    self.flange_plate.Innerheight = round_down(self.total_height_of_inner_plate / 2, 5)
                    self.flange_weld.Innerheight = round_down(
                        (self.flange_plate.Innerheight - 2 * self.flange_weld.size), 5)
        ...
        def cap_blockcheck_web_axial(self):
            ...
            for self.web_plate.thickness_provided in self.web_plate_thickness_possible:
                ...
                while design_status_block_shear == False:
                    ...
                    Avg = 2 * (self.available_long_web_length + self.web_weld.size) * self.section.web_thickness
                    Avn = 2 * (self.available_long_web_length + self.web_weld.size) * self.section.web_thickness
