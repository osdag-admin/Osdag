...
class PopupDialog(QDialog):
    ...
    def init_ui(self):
        self.label = QLabel("Available:", self)
        self.label.setGeometry(QRect(20, 20, 150, 30))

        self.label_2 = QLabel("Selected:", self)
        self.label_2.setGeometry(QRect(int(scale * 320), 20, 150, 30))

        self.listWidget = My_ListWidget(self)
        self.listWidget.setGeometry(QRect(20, 50, int(scale*180), int(scale*300)))
        self.listWidget.setSelectionMode(QAbstractItemView.ExtendedSelection)
        self.listWidget.itemDoubleClicked.connect(self.move_to_selected)

        self.listWidget_2 = My_ListWidget(self)
        self.listWidget_2.setGeometry(QRect(int(scale*320), 50, int(scale*180), int(scale*300)))
        ...
        self.pushButton.setGeometry(QRect(int(scale*225), int(scale*140), int(scale*70), int(scale*30)))

        self.pushButton_2 = QPushButton(">", self)
        self.pushButton_2.setGeometry(QRect(int(scale*225), int(scale*180), int(scale*70), int(scale*30)))

        self.pushButton_3 = QPushButton("<", self)
        self.pushButton_3.setGeometry(QRect(int(scale*225), int(scale*220), int(scale*70), int(scale*30)))

        self.pushButton_4 = QPushButton("<<", self)
        self.pushButton_4.setGeometry(QRect(int(scale*225), int(scale*260), int(scale*70), int(scale*30)))

        self.pushButton_5 = QPushButton("Submit", self)
        self.pushButton_5.setGeometry(QRect(int(scale*190), int(scale*400), int(scale*140), int(scale*35)))
...
class PlateGirderWelded(Member):
    ...
    def max_defl_change(self, input):
        if input[0] in ['Highway Bridge','Railway Bridge']:
        ...
        elif input[0] == 'Other Building':
            if input[1] == 'Live load':
                if input[2] == 'Floor and roof':
                    if input[3] == 'Elements not susceptible to cracking':
            ...
            if input[2] == 'Purlin and Girts' and input[1] == 'Live load':
                if input[3] == 'Elastic cladding':
                    return {KEY_MAX_DEFL : VALUES_MAX_DEFL[5]}
                else:
                    return {KEY_MAX_DEFL : VALUES_MAX_DEFL[6]}
            elif input[2] == 'Simple span' and input[1] == 'Live load':
                if input[3] == 'Elastic cladding':
                    return {KEY_MAX_DEFL : VALUES_MAX_DEFL[7]}
                else:
                    return {KEY_MAX_DEFL : VALUES_MAX_DEFL[3]}
            elif input[2] == 'Cantilever span' and input[1] == 'Live load':
                if input[3] == 'Elastic cladding':
                    return {KEY_MAX_DEFL : VALUES_MAX_DEFL[8]}
                else:
                    return {KEY_MAX_DEFL : VALUES_MAX_DEFL[5]}
            elif input[2] == 'Rafter Supporting' and input[1] == 'Live load':
                if input[3] == 'Profiled Metal sheeting':
                    return {KEY_MAX_DEFL : VALUES_MAX_DEFL[6]}
                else:
                    return {KEY_MAX_DEFL : VALUES_MAX_DEFL[7]}
            elif input[2] == 'Gantry' and input[1] == 'Live load':
                if input[1] == 'Crane Load(Manual operation)':
                    return {KEY_MAX_DEFL : VALUES_MAX_DEFL[9]}
                elif input[1] == 'Crane load(Electric operation up to 50t)':
    ...
    def Int_stiffener_thickness_customized(self, input):
        selected_items = []
        if input[0] == 'All':
    ...
    def Long_stiffener_thickness_customized(self, input):
        selected_items2 = []
        if input[0] == 'All':
    ...
    def fn_torsion_warping(self, input):
        # print( 'Inside fn_torsion_warping', self)
        if input[0] == Torsion_Restraint1:
            return Warping_Restraint_list
        elif input[0] == Torsion_Restraint2:
            return [Warping_Restraint5]
        else:
            return [Warping_Restraint5]

    def axis_bending_change(self, input):
        design = input[0]
        # print( 'Inside fn_supp_image', self)
        if input[0] == KEY_DISP_DESIGN_TYPE_FLEXURE:
            return ['NA']
        else:
    ...
    def customized_dimensions(self, input):
        conn = input[0]
        if conn == "Customized":
    ...
    def customized_dimensions_1(self, input):
        conn = input[0]
        if conn == "Customized":
    ...
    def customized_dimensions_2(self, input):
        conn = input[0]
        if conn == "Customized":
    ...
    def customized_dims(self, input):
        conn = input[0]
        if conn == "Customized":
            return True
        else:
            return False
    def customized_options(self, input):
        conn = input[0]
        if conn == "Customized":
    ...
    def customized_dimensions_cst(self, input):
        conn = input[0]
        if conn == "Optimized":
    ...
    def customized_dims_cst(self, input):
        conn = input[0]
        if conn == "Optimized":
            return True
        else:
    ...
    def pop_up_bounds(self, input):
        if input[0] == "Bound Values":
    ...
    def warning_majorbending(self, input):
        # print(self)
        if input[0] == VALUES_SUPP_TYPE_temp[2]:
            return True
        # elif input[0] == VALUES_SUPP_TYPE_temp[0] or input[0] == VALUES_SUPP_TYPE_temp[1] :
        #     return True
        else:
            return False

    def output_modifier(self, input):
        # print(self)
        if input[0] == VALUES_SUPP_TYPE_temp[2]:
            return False
        # elif input[0] == VALUES_SUPP_TYPE_temp[0] or input[0] == VALUES_SUPP_TYPE_temp[1] :
    ...
    def output_modifier_long_stiffener(self, input):
        if input[0] == 'Thin we':
            return False
        else:
            return True
        
    def output_modifier2(self, input):
        # print(self)
        if input[0] == 'Thin Web with ITS':
            return False
        # elif input[0] == VALUES_SUPP_TYPE_temp[0] or input[0] == VALUES_SUPP_TYPE_temp[1] :
        #     return True
        else:
    ...
    def section_classification(self,design_dictionary):
        ...
        self.effective_length_beam(design_dictionary, self.length)
    ...
    def beta_value(self, design_dictionary, section_class):
        self.plast_sec_mod_z = Unsymmetrical_I_Section_Properties.calc_PlasticModulusZ(self.total_depth,self.top_flange_width,self.bottom_flange_width,
                                                    self.web_thickness,self.top_flange_thickness,self.bottom_flange_thickness,self.epsilon)
        self.elast_sec_mod_z =Unsymmetrical_I_Section_Properties.calc_ElasticModulusZz(self.total_depth,self.top_flange_width,self.bottom_flange_width,
        ...
        if self.plast_sec_mod_z >= self.Zp_req:
            pass
            # self.logger.info( 'self.section_property.plast_sec_mod_z More than Requires',self.plast_sec_mod_z,self.Zp_req)
    ...
    def moment_capacity_laterally_supported(self, V,Zp, Ze, Fy, gamma_m0, D, tw, tf_top, tf_bot, section_class) :
        ...
        if V > 0.6 * self.V_d: #high shear(Mdv calculation for high shear. Naming kept Md for compatibility)
            ...
            self.Md = self.calc_Mdv(V, self.V_d, Zp, Ze, Fy, gamma_m0, D, tw, tf_top, tf_bot)
            # print("Md", self.Md)
        else: #low shear
        ...
        try:
            ...
            if isinstance(self, PlateGirderWelded):
                if not self.web_crippling_warning_logged:
                    self.logger.error(f"Error in web crippling check during optimization: {str(e)}")
    ...
    def moment_capacity_laterally_unsupported(self, E, LLT, D,
        ...
        G = 0.769 * 10 ** 5
        Kw = self.get_K_from_warping_restraint(self.warping)
        Iy = Unsymmetrical_I_Section_Properties.calc_MomentOfAreaY(self.total_depth, self.top_flange_width,
                                                                   self.bottom_flange_width, self.web_thickness,
                                                                   self.top_flange_thickness,
                                                                   self.bottom_flange_thickness)
        self.It = Unsymmetrical_I_Section_Properties.calc_TorsionConstantIt(self.total_depth, self.top_flange_width,
                                                                       self.bottom_flange_width, self.web_thickness,
                                                                       self.top_flange_thickness,
                                                                       self.bottom_flange_thickness)
        self.Iw = Unsymmetrical_I_Section_Properties.calc_WarpingConstantIw(self.total_depth, self.top_flange_width,
                                                                       self.bottom_flange_width, self.web_thickness,
                                                                       self.top_flange_thickness,
                                                                       self.bottom_flange_thickness)
        ...
        # Mcr calc
        yg = D / 2
        yj = self.calc_yj(Bf_top, tf_top, Bf_bot, tf_bot, D)
        K_value = 0
        ...
        if shear_force > 0.6 * self.V_d:  # high shear(Mdv calculation for high shear. Naming kept Md for compatibility)
            self.Md = self.calc_Mdv_lat_unsupported(self.load.shear_force, self.V_d, self.plast_sec_mod_z,
            ...
            self.Md = self.bending_check_lat_unsupported(self.beta_b_lt, self.plast_sec_mod_z,
    ...
    def effective_length_beam(self, design_dictionary, length):
        if design_dictionary[KEY_LENGTH_OVERWRITE] == 'NA':
            ...
            try:
                ...
                self.logger.warning("Invalid Effective Length Parameter.")
                self.logger.info('Effective Length Parameter is set to default: 1.0')
    ...
    def check_web_crippling(self, N, tw, fy, d):
        ...
        try:
            ...
            if any(val <= 0 for val in [N, tw, fy, d]):
                ...
                if isinstance(self, PlateGirderWelded):
                    if not self.web_crippling_warning_logged:
                        self.logger.warning("Invalid input parameters for web crippling check")
                        self.web_crippling_warning_logged = True
                else:
                    if not PlateGirderWelded._web_crippling_warning_logged:
                        self.logger.warning("Invalid input parameters for web crippling check")
            ...
            if P_w >= self.load.shear_force:
                ...
                if isinstance(self, PlateGirderWelded):
                    if not self.web_crippling_warning_logged:
                        self.logger.warning(f"Web crippling resistance ({P_w:.2f} N) is less than factored load ({self.load.shear_force:.2f} N)")
                        self.web_crippling_warning_logged = True
                else:
                    if not PlateGirderWelded._web_crippling_warning_logged:
                        self.logger.warning(f"Web crippling resistance ({P_w:.2f} N) is less than factored load ({self.load.shear_force:.2f} N)")
            ...
            if isinstance(self, PlateGirderWelded):
                if not self.web_crippling_warning_logged:
                    self.logger.error(f"Error in web crippling calculation: {str(e)}")
                    self.web_crippling_warning_logged = True
            else:
                if not PlateGirderWelded._web_crippling_warning_logged:
                    self.logger.error(f"Error in web crippling calculation: {str(e)}")
    ...
    def end_panel_stiffener_calc(self,
            ...
            if web_height/tw > 200:  # Slender web condition requiring additional checks
                try:
                    ...
                    if not web_crippling_check:
                        ...
                        if isinstance(self, PlateGirderWelded):
                            if not self.web_crippling_warning_logged:
                                self.logger.warning("Web crippling check failed as per IS 800:2007. Consider increasing web thickness or adding stiffeners.")
                                self.web_crippling_warning_logged = True
                        else:
                            if not PlateGirderWelded._web_crippling_warning_logged:
                                self.logger.warning("Web crippling check failed as per IS 800:2007. Consider increasing web thickness or adding stiffeners.")
                    ...
                    if isinstance(self, PlateGirderWelded):
                        if not self.web_crippling_warning_logged:
                            self.logger.error(f"Error during web crippling check: {str(e)}")
                            self.web_crippling_warning_logged = True
                    else:
                        if not PlateGirderWelded._web_crippling_warning_logged:
                            self.logger.error(f"Error during web crippling check: {str(e)}")
        ...
        #
        # # Torsional restraint
        # ry = Unsymmetrical_I_Section_Properties.calc_RadiusOfGyrationY(total_depth, Bf_top, Bf_bot, tw, tf_top, tf_bot)
    ...
    def evaluate_deflection_kNm_mm(self,
        ...
        I = Unsymmetrical_I_Section_Properties.calc_MomentOfAreaZ(self.total_depth,self.top_flange_width,self.bottom_flange_width,self.web_thickness,self.top_flange_thickness,self.bottom_flange_thickness)
        delta = self.deflection_from_moment_kNm_mm(M_kNm, L, E, I, case)
    ...
    def weld_leg_from_q_with_cl10(self,
        ...
        f_wd = IS800_2007.cl_10_5_7_1_1_fillet_weld_design_stress(ultimate_stresses)  # MPa
    ...
    def design_welds_with_strength_web_to_flange(self,
        ...
        # compute shear flows
        sf = self.shear_stress_unsym_I(V_ed, b_ft, t_ft, b_fb, t_fb, t_w, h_w)
        ...
        a_top = round_up(max(self.weld_leg_from_q_with_cl10(
                                            sf['q_top_kN_per_mm'], ultimate_stresses
                                            ), min_weld_legtop) and min(self.weld_leg_from_q_with_cl10(
                                            sf['q_top_kN_per_mm'], ultimate_stresses
                                            ), max_weld_legtop),1)

        a_bot = round_up(max(self.weld_leg_from_q_with_cl10(
                                            sf['q_bot_kN_per_mm'], ultimate_stresses
                                            ), min_weld_legbot) and min(self.weld_leg_from_q_with_cl10(sf['q_bot_kN_per_mm'], ultimate_stresses
                                            ), max_weld_legbot),1)
    ...
    def weld_for_end_stiffener(self, t_st, b_st, V_ed, V_unstf, D, t_ft, t_fb, tw, ultimate_stresses):
        ...
        # weld legs using cl.10 strength
        weld_stiff = self.weld_leg_from_q_with_cl10(q_each, ultimate_stresses)
    ...
    def design_longitudinal_stiffeners(self, d, tw, c, eps_w, second_stiffener=False):
        ...
        c = float(c)
        tw = float(tw)
        d_na= Unsymmetrical_I_Section_Properties.calc_centroid(self.total_depth, self.top_flange_width, self.bottom_flange_width, self.web_thickness, self.top_flange_thickness, self.bottom_flange_thickness)
        # print('d_na', d_na)
    ...
    def optimized_method(self, design_dictionary, is_thick_web, is_symmetric):
        ...
        init_real = self.generate_first_particle(
            self.length, self.load.moment, self.material.fy,
        ...
        def constraint(particle):
            ...
            max_ratio, slender_ok, thickness_ok = self.design_check_optimized_version(design)
        ...
        # def obj_fn_single(x):
        #     sec = Section()
        #     self.assign_particle_to_section(x, variable_list, sec)
        #     area = ((self.top_flange_thickness * self.top_flange_width) +
        #             (self.bottom_flange_thickness * self.bottom_flange_width) +
        #             (self.web_thickness * (self.total_depth - self.top_flange_thickness - self.bottom_flange_thickness)))
        #     volume = area * self.length  # mm³
        ...
        #best_pos, best_cost = pso(obj_fn_single, lb, ub, f_ieqcons=constraint, swarmsize=100, maxiter=500, debug=True, init_pos=self.generate_first_particle(self.length, self.load.moment, self.material.fy, is_thick_web, is_symmetric))
        margins = constraint(best_pos)
        best_section = Section()
        self.assign_particle_to_section(best_pos, variable_list, best_section)
        ...
        # self.logger.info("NEW PSO RUNNING")
    ...
    def objective_function(self,x, variable_list,design_dictionary,is_symmetric,is_thick_web):
        ...
        for particle in x:
            sec = Section()
            self.assign_particle_to_section(particle, variable_list, sec)
            cost = self.run_design_checks(sec,design_dictionary,is_symmetric,is_thick_web)
            costs.append(cost)
    ...
    def run_design_checks(self,section,design_dictionary,is_symmetric,is_thick_web):
        ...
        # placeholder penalty
        penalty = 0
        maxiratio, slendercheck, thicknesscheck = self.design_check_optimized_version(design_dictionary)
    ...
    def design_check(self,design_dictionary):
        ...
        if self.design_flag == False:
            ...
            if self.web_philosophy == 'Thick Web without ITS':
                self.design_flag2 = self.min_web_thickness_thick_web(self.eff_depth,self.web_thickness,self.epsilon,"no_stiffener",0)
                
                if self.design_flag2 == True:
                    
                    #shear check
                    if self.shear_capacity_laterally_supported_thick_web(self.material.fy,self.gamma_m0,self.total_depth,self.web_thickness,self.top_flange_thickness,self.bottom_flange_thickness):
                    ...
                    #web buckling check
                    if self.web_buckling_laterally_supported_thick_web(self.material.fy,self.gamma_m0,self.total_depth,self.web_thickness,self.top_flange_thickness,self.bottom_flange_thickness,self.material.modulus_of_elasticity,self.b1):
                    ...
                    #web crippling check
                    if self.web_crippling_laterally_supported_thick_web(self.material.fy,self.gamma_m0,self.web_thickness,self.top_flange_thickness,self.b1):
                    ...
                    if self.support_type == 'Major Laterally Supported':
                        ...
                        #moment check supp
                        if self.moment_capacity_laterally_supported(self.load.shear_force,self.plast_sec_mod_z,self.elast_sec_mod_z,self.material.fy,self.gamma_m0,self.total_depth,self.web_thickness,self.top_flange_thickness,self.bottom_flange_thickness,self.section_class):
                        ...
                        #moment check unspp
                        if self.moment_capacity_laterally_unsupported(self.material.modulus_of_elasticity,self.effective_length,self.total_depth,self.top_flange_thickness,self.bottom_flange_thickness,self.top_flange_width,self.bottom_flange_width,self.web_thickness,self.loading_case,self.gamma_m0,self.material.fy,self.load.shear_force):
                            # print("M", self.Md)
                            self.momentchecks = True
                            self.logger.info("Moment Check passed")
                        else:
                ...
                if self.stiffener_type != "transverse_only":
                    ...
                    if self.design_longitudinal_stiffeners(self.eff_depth, self.web_thickness, self.c,self.epsilon, second_stiffener):
                        self.logger.info("Longitudinal Stiffener Check passed")
                    else:
                        self.logger.error("Longitudinal Stiffener Check failed")

                if self.c == 'NA':
                    self.logger.error("c value not provided")
                    self.c = 0
                else:
                    self.c = float(self.c)
                # print("c value",self.c)
                self.design_flag2 = self.min_web_thickness_thick_web(self.eff_depth,self.web_thickness,self.epsilon,self.stiffener_type,self.c)
                if self.design_flag2 == True:
                    ...
                    if design_dictionary[KEY_ShearBucklingOption] == 'Simple Post Critical':
                        #shear check
                        if self.shear_buckling_check_simple_postcritical(self.eff_depth,self.total_depth,self.top_flange_thickness,self.bottom_flange_thickness,self.web_thickness,self.load.shear_force,self.c):
                            self.shearflag1 = True
                            self.logger.info("Shear Check passed")
                        else:
                            self.logger.info("Shear Check Failed, add end stiffeners")
                            if self.end_panel_stiffener_calc(self.top_flange_width, self.bottom_flange_width,
                        ...
                        if self.shear_buckling_check_intermediate_stiffener(self.eff_depth,self.web_thickness,self.c,self.epsilon,self.IntStiffThickness,self.IntStiffnerwidth,self.load.shear_force,self.gamma_m0,self.material.fy,self.material.modulus_of_elasticity):
                            self.shearflag2 = True
                        ...
                        if self.shear_buckling_check_tension_field(self.eff_depth,self.total_depth,self.top_flange_thickness,self.bottom_flange_thickness,self.web_thickness,self.c):
                            self.shearflag1 = True
                            self.logger.info("Shear Buckling Check passed")
                        else:

                            self.logger.error("Shear Buckling Check failed, provide end panel stiffeners")
                            if self.tension_field_end_stiffener(self.eff_depth, self.web_thickness, self.material.fy,
                        ...
                        if self.tension_field_intermediate_stiffener(self.eff_depth, self.web_thickness, self.c, self.epsilon, self.IntStiffThickness, self.IntStiffnerwidth, self.load.shear_force, self.gamma_m0, self.material.fy, self.material.modulus_of_elasticity):
                            self.shearflag2 = True
                            self.logger.info("Shear Buckling Check passed with intermediate stiffeners")
                        else:
                            self.shearflag2 = False
        ...
        #deflection checks
        if self.evaluate_deflection_kNm_mm(self.load.moment, self.length, self.material.modulus_of_elasticity, self.loading_case, self.deflection_criteria):
    ...
    def design_check_optimized_version(self,design_dictionary):
        ...
        if self.design_flag == False:
            ...
            if self.web_philosophy == 'Thick Web without ITS':
                ...
                if self.stiffener_type != "transverse_only":
                    ...
                    if self.stiffener_type == "transverse_and_two_longitudinal_neutral":
                        second_stiffener = True
                    if self.design_longitudinal_stiffeners(self.eff_depth, self.web_thickness, self.c,
                    ...
                    if design_dictionary[KEY_ShearBucklingOption] == 'Simple Post Critical':
                        ...
                        if self.shear_buckling_check_tension_field(self.eff_depth,self.total_depth,self.top_flange_thickness,self.bottom_flange_thickness,self.web_thickness,self.c):
                            ...
                            if self.tension_field_end_stiffener(self.eff_depth, self.web_thickness, self.material.fy,
                                ...
                                self.shearflag1 = False
                                # self.logger.error("Tension Field Check failed, increase stiffener thickness")
                        if self.tension_field_intermediate_stiffener(self.eff_depth, self.web_thickness, self.c,
        ...
        if self.evaluate_deflection_kNm_mm(self.load.moment, self.length, self.material.modulus_of_elasticity, self.loading_case, self.deflection_criteria):
            ...
            self.defl_check = False
            # self.logger.error("Deflection Check failed")d
    ...
    def optimized_method_working(self,design_dictionary,is_thick_web, is_symmetric):
        ...
        objective_func=lambda swarm: self.objective_function(swarm, variable_list,design_dictionary,is_symmetric,is_thick_web),
        iters=100
    )
    ...
    def final_format(self, design_dictionary):
        ...
        self.atop, self.abot= self.design_welds_with_strength_web_to_flange(self.load.shear_force, self.top_flange_width, self.top_flange_thickness, self.bottom_flange_width, self.bottom_flange_thickness, self.web_thickness, self.eff_depth, [self.material.fu])
        self.weld_stiff = self.weld_for_end_stiffener(self.end_stiffthickness, self.end_stiffwidth, self.load.shear_force, self.V_d, self.total_depth, self.top_flange_thickness, self.bottom_flange_thickness, self.web_thickness, [self.material.fu])
        self.design_status = True

    def save_design(self, popup_summary):
        # print("\n\n\n\n Enterend save design")
        self.logger.info(" :=========Start Of design Saving Button pressed===========")
