...
class ISection(object):
    ...
    def create_model(self):
        ...
        if self.notch_obj != None:
            uDir = numpy.array([-1.0, 0.0, 0])
            wDir = numpy.array([0.0, 1.0, 0.0])
            shift_origin = self.D / 2.0 * self.vDir + self.notch_obj.width / 2.0 * self.wDir + self.B / 2.0 * -self.uDir  # + self.notch_obj.width* self.wDir + self.T/2.0 * -self.uDir
        ...
        + self.notch_obj.height / 2.0 * self.uDir
