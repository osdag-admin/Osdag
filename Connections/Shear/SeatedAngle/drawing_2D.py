...
class Seat2DCreatorFront(object):
    ...
    def call_CFBF_front(self, file_name):
        ...
        ptA2 = ptA1 + ((self.data_object.end_dist + self.data_object.beam_flange_thk + self.data_object.beam_R1 + 3) + 115) * np.array(
            [0, -1])
...
class Seat2DCreatorSide(object):
    ...
    def call_CWBF_side(self, file_name):
        ...
        dwg = svgwrite.Drawing(file_name, size=('100%', '100%'), viewBox=('-10 -100 1500 1020'))
        ...
        dwg.add(dwg.polyline(points=[self.SWA, self.SWE, self.SWF, self.SWB, self.SWC, self.SWG, self.SWH, self.SWD, self.SWA], stroke='blue',
                             fill='none', stroke_width=2.5))
        ...
        # Weld Information
        weldPt = np.array(pitchPts[len(pitchPts) - 1]) + self.data_object.pitch / 2 * np.array([0, -1]) + (self.data_object.plate_thick +
                                                                                                           4) * np.array([-1, 0])
        theta = 45
