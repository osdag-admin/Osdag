...
class ReportCustomizationDialog(QDialog):
    ...
    def load_existing_or_generate_report(self):
        ...
        try:
            ...
            if self.existing_tex_file and os.path.exists(self.existing_tex_file):
                ...
                # No LaTeX available
                QMessageBox.critical(self, "Error", "No LaTeX content available for customization")

        except Exception as e:
            print(f"ERROR in load_existing_or_generate_report: {e}")
            QMessageBox.critical(self, "Error", f"Failed to load/generate report: {e}")
    ...
    def parse_and_compile(self):
        ...
        if sections:
            ...
            QMessageBox.warning(self, "Warning", "No sections found in LaTeX content")
    ...
    def compile_pdf(self):
        ...
        try:
            ...
            if result.returncode == 0 and os.path.exists(pdf_file):
                ...
                QMessageBox.warning(self, "Compilation Failed", error_msg)
        except subprocess.TimeoutExpired:
            error_msg = "PDF compilation timed out (>30s)\n\nThis might be due to:\n• LaTeX not installed\n• Missing packages\n• Complex LaTeX content"
            print(f"TIMEOUT: {error_msg}")
            QMessageBox.warning(self, "Compilation Timeout", error_msg)
        except FileNotFoundError:
            error_msg = "pdflatex not found\n\nInstall LaTeX distribution:\n• Windows: MiKTeX or TeX Live\n• Linux: texlive-full\n• macOS: MacTeX"
            print(f"ERROR: {error_msg}")
            QMessageBox.warning(self, "LaTeX Not Found", error_msg)
        except Exception as e:
    ...
    def open_latest_pdf(self):
        ...
        if self.latest_pdf and os.path.exists(self.latest_pdf):
            ...
            QMessageBox.information(self, "No PDF", "No PDF generated yet. Please compile first.")

    def save_pdf(self):
        """Save customized PDF"""
        if not self.latest_pdf or not os.path.exists(self.latest_pdf):
            QMessageBox.warning(self, "No PDF", "No PDF to save. Please compile first.")
            return
