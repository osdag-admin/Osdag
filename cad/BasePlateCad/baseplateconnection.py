...
class BasePlateCad(object):
    def __init__(self, column, nut_bolt_array, bolthight, baseplate, weldAbvFlang, weldBelwFlang, weldSideWeb, concrete, gusset, stiffener, type):
    ...
    def createBasePlateGeometry(self):
        ...
        if self.type == 'gusset':
            ...
            stiffener2OriginL = numpy.array([self.stiffener.T/2-stiffener_gap, self.column.D/2+self.stiffener.L/2+self.gusset.T, self.stiffener.W/2])
            ...
            stiffener3OriginL = numpy.array([-self.stiffener.T/2+stiffener_gap, -(self.column.D/2+self.stiffener.L/2+self.gusset.T), self.stiffener.W/2])
            ...
            stiffener4OriginL = numpy.array([-self.stiffener.T/2-stiffener_gap, -(self.column.D/2+self.stiffener.L/2+self.gusset.T), self.stiffener.W/2])
    ...
    def get_plate_connector_models(self):
        ...
        if self.type == 'gusset':
            plate_list = [self.baseplateModel, self.gusset1Model, self.gusset2Model, self.stiffener1Model, self.stiffener2Model, self.stiffener3Model, self.stiffener4Model]
            plate = plate_list[0]
...
if __name__ == '__main__':
    ...
    basePlate = BasePlateCad(column, nut_bolt_array, bolthight, baseplate, weldAbvFlang, weldBelwFlang, weldSideWeb, concrete, gusset, stiffener, type)
