...
class BeamWebBeamWeb(object):
    ...
    def __init__(self,column,beam,notch,Fweld,plate,nutBoltArray):
        self.column = column
        self.beam = beam
        self.notch = notch
        self.weldLeft = Fweld
    ...
    def createBeamGeometry(self):
        ...
        origin2 = self.column.secOrigin + (-shiftOrigin) * self.column.vDir + (self.column.t/2 * self.column.uDir) + (self.column.length/2 * self.column.wDir) + (self.plate.T * self.column.uDir) 
...
#         self.beam.place(origin2, uDir, wDir)
    ...
    def createPlateGeometry(self):
        plateOrigin = self.beam.secOrigin + (self.plate.W/2)*(-self.beam.uDir) + (self.plate.T/2)*(-self.beam.wDir) + (self.beam.D/2 - self.notch.height - self.plate.L/2)*(self.beam.vDir)    #(self.beam.D/2 - self.notch.height - self.plate.L/2) * (-self.beam.vDir) + (self.plate.W/2) * (self.beam.uDir) + (self.plate.T/2) * (-self.beam.wDir)
    ...
    def createFilletWeldGeometry(self):
        ...
        filletWeld2Origin = (filletWeld1Origin + self.beam.t * (-self.weldLeft.vDir))
