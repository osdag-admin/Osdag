...
def extract_simple_plated(cad_obj):
    ...
    # Packing Plates (for double-cover Bolted Butt Joint with unequal thickness)
    for attr in ['packing1', 'packing2']:
        if hasattr(cad_obj, attr):
            val = getattr(cad_obj, attr)
            if val is not None:
                plates.append(val)
            ...
            if val is not None:
                ...
                if v_type in ('FilletWeld', 'GrooveWeld', 'Weld'):
                    ...
                    if v_type == 'Plate' or v_type == 'StiffenerPlate': 
                        if val not in plates: plates.append(val)
                    elif v_type == 'Angle': 
                        if val not in plates: plates.append(val)
                    else: 
...
def extract_truss_connection(cad_obj):
    ...
    if getattr(cad_obj, 'intermittentConnection_Model', None) is not None:
        ...
        if ic is not None:
            if hasattr(ic, 'plates'): plates.extend(ic.plates)
...
def extract_metadata(module_obj, design_dict=None):
    ...
    if design_dict:
        ...
        for k, v in design_dict.items():
            ...
            if 'Load.Axial Force' in k_str or 'Load.AxialForce' in k_str:
                try: meta['AxialForce_kN'] = float(v)
                except: pass
            elif 'Load.Shear Force' in k_str or 'Load.ShearForce' in k_str:
                try: meta['ShearForce_kN'] = float(v)
                except: pass
            elif 'Load.Moment' in k_str:
