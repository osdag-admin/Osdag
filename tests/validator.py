...
def extract_multiple_values(file_path):
    ...
    # Fix: Use absolute fallback if path doesn't exist
    if not os.path.isfile(file_path):
        fallback_dir = r"C:\Visual Studio Code\FOSSEE OSDAG\Osdag\tests\osi_files"
        file_path = os.path.join(fallback_dir, os.path.basename(file_path))
...
def validate_osi(file_path):
    ...
    if not bolt_dia_check or "" in bolt_dia_check:
        results["Bolt.Diameter"] = ""  # Skip check
    else:
