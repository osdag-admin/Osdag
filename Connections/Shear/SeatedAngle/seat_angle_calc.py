...
class SeatAngleCalculation(ConnectionCalculations):
    ...
    def bolt_design(self):
        ...
        if self.is_hsfg == False:
            ...
            self.bolt_shear_capacity = ConnectionCalculations.bolt_shear_hsfg(self.bolt_diameter, self.bolt_fu,
                                                                              self.mu_f,
                                                                              self.n_e,
                                                                              self.bolt_hole_type)
    ...
    def seat_angle_connection(self, input_dict):
        ...
        if self.gauge < self.min_gauge:
            ...
            if self.num_cols == 1:
                ...
                logger.warning(": Two bolts of selected diameter cannot fit in the available length of seated angle")
        ...
        #         self.safe = False
        #         logger.error(": Detailing failure")
        #         logger.warning(": Bolt intersects the column root radius")
        ...
        # elif self.num_rows == 1 and self.end_dist > self.max_end_dist:
        #     logger.warning(": End distance should be less than  %2.2f mm " % self.max_end_dist)
        #     logger.warning(": Reduce the seated angle leg A length, to avoid a detailing failure")
        #     self.num_rows = 2
        #     add code for updating distances
