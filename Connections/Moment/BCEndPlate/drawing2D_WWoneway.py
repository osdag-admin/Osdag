...
########
########
class OnewayEndPlate_WW(object):
...
class Oneway2DFront(object):
	...
	#	ptS1y = self.data_object.column_length_L1/2 - self.data_object.beam_depth_D2/2 +\
        ...
        ptA8x = ptA5x
        ptA8y = ptA5y + self.data_object.column_length_L1
    ...
    def call_Oneway_front(self, filename):
        ...
        dwg.add(dwg.line(self.A6, self.A7).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
        dwg.add(dwg.line(self.A6, self.P1).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
        dwg.add(dwg.line(self.P4, self.A7).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
        ...
        dwg.add(dwg.line(self.AA10, self.AA11).stroke('black', width=2.5, linecap='square'))
...
class Oneway2DTop(object):
    ...
    def call_Oneway_top(self, filename):
        ...
        dwg.add(dwg.rect(insert=self.A4,
                         ...
                         stroke_width=1.0))
        pattern.add(dwg.path(d="M 0,1 l 6,0", stroke='#000000', stroke_width=2.5))
        dwg.add(dwg.rect(insert=self.A9 - self.data_object.weld_plate_thickness_p2 * np.array([1, 0]), size=(float
        (self.data_object.weld_plate_thickness_p2),
        ...
        pt_a2 = pt_a1 + (
                    self.data_object.column_width_B1 + self.data_object.beam_length_L2 + self.data_object.plate_thickness_p1) * np.array(
...
class Oneway2DSide(object):
    ...
    def call_Oneway_side(self, filename):
		...
		"""
        wd = (int(self.data_object.column_depth_D1 + 600 + 300))
        ht = (int(self.data_object.column_length_L1 + 500 + 300))
        dwg = svgwrite.Drawing(filename, size=('100%', '100%'), viewBox=(
            '-600 -500 {} {}').format(wd, ht))
        ...
        dwg.add(dwg.polyline(points=[self.P1, self.P2, self.P3, self.P4, self.P1], stroke='black', fill='none',
                             stroke_width='2.5'))
        dwg.add(dwg.polyline(points=[self.AA1, self.AA2, self.AA3, self.AA4, self.AA1], stroke='black', fill='none', stroke_width=2.5))
        ...
        for i in range(1, (bitfr + 1)):
            ...
            for j in range(1, (nofc + 1)):
                if self.data_object.no_of_bolts == 6:
                    pt = self.P1 + (self.data_object.plate_length_L1 - self.data_object.flange_projection - self.data_object.beam_depth_D2 + self.data_object.flange_thickness_T2 + self.data_object.Lv) * np.array(
        ...
        for i in range(1, (bibfr + 1)):
            ...
            for j in range(1, (nofc + 1)):
                pt = self.P4 + (self.data_object.flange_projection + self.data_object.flange_thickness_T2 + self.data_object.Lv) * np.array(
            ...
            if self.data_object.no_of_bolts == 6:
                ...
                point3 = np.array(pt_inside_top_column_list[1][1])
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "right",
                          "endlinedim": 10, "arrowlen": 20}
        ...
        # ------------------------------------------  View details-------------------------------------------
        ptx = self.P4 * np.array([0, 1]) + 400 * np.array([0, 1])
