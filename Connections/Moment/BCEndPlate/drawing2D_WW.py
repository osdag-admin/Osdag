...
class Web_to_Web(object):
    ...
    def __init__(self, input_dict, output_dict, column_data, beam_data, folder):
        ...
        self.input_dict = input_dict['Member']['EndPlate_type']
        ...
        self.stiffener_weldsize = output_dict['Stiffener']['Weld']
        self.stiffener_weld = 0

        self.no_of_columns = 2
        self.no_of_bolts = output_dict['Bolt']['NumberOfBolts']
        if input_dict['Member']['EndPlate_type'] == "Extended both ways":
        ...
        elif input_dict['Member']['EndPlate_type'] == "Extended one way":
    ...
    def save_to_svg(self, filename, view):
		...
		"""
        beamcolumn_flange_web_2d_front = BC_WW_2DFront(self)
        beamcolumn_flange_web_2d_top = BC_WW_2DTop(self)
        beamcolumn_flange_web_2d_side = BC_WW_2DSide(self)
        if view == "Front":
            beamcolumn_flange_web_2d_front.call_front(filename)
        elif view == "Top":
            beamcolumn_flange_web_2d_top.call_top(filename)
        elif view == "Side":
            beamcolumn_flange_web_2d_side.call_side(filename)
        else:
...
class BC_WW_2DFront (object):
    ...
    def __init__(self, extnd_common_object):
        ...
        ptA2x = ptA1x + self.data_object.column_width_B1
        ptA2y = ptA1y
        ...
        ptA5x = ptA1x + self.data_object.column_width_B1/2 - self.data_object.web_thickness_tw1/2
        ptA5y = ptA1y
        self.A5 = np.array([ptA5x, ptA5y])

        ptA6x = ptA5x + self.data_object.web_thickness_tw1
        ptA6y = ptA5y
        self.A6 = np.array([ptA6x, ptA6y])

        ptA7x = ptA6x
        ptA7y = ptA6y + self.data_object.column_length_L1
        self.A7 = np.array([ptA7x, ptA7y])

        ptA8x = ptA5x
        ptA8y = ptA5y + self.data_object.column_length_L1  # (self.data_object.beam_depth_D1 - self.data_object.flange_thickness_T1)
        self.A8 = np.array([ptA8x, ptA8y])

        # self.Q = self.A6 + self.data_object.web_weld_thickness * np.array([-1, 0]) dont know


        if self.data_object.input_dict == "Extended one way":
            # ================ Connecting Plate ==================
            ...
            downward to the origin is considered as +ve Y axis
            """
            ptP4x = ptA1x + self.data_object.column_width_B1 / 2 + self.data_object.web_thickness_tw1 / 2
            ...
            ptAA1y = ptP2y + self.data_object.plate_length_L1 - self.data_object.flange_projection - self.data_object.beam_depth_D2
            self.AA1 = np.array([ptAA1x, ptAA1y])

            ptAA2x = ptAA1x + self.data_object.column_width_B1 / 2 - self.data_object.plate_thickness_p1 - self.data_object.web_thickness_tw1 / 2
            ptAA2y = ptAA1y
            self.AA2 = np.array([ptAA2x, ptAA2y])

            ptAA3x = ptAA1x + self.data_object.beam_length_L2
            ptAA3y = ptAA1y
            self.AA3 = np.array([ptAA3x, ptAA3y])

            ptAA4x = ptAA3x
            ptAA4y = ptAA3y + self.data_object.beam_depth_D2
            self.AA4 = np.array([ptAA4x, ptAA4y])

            ptAA5x = ptAA4x - self.data_object.beam_length_L2 + self.data_object.column_width_B1 / 2 - self.data_object.plate_thickness_p1 - self.data_object.web_thickness_tw1 / 2
            ptAA5y = ptAA4y
            self.AA5 = np.array([ptAA5x, ptAA5y])

            ptAA6x = ptAA4x - self.data_object.beam_length_L2
            ptAA6y = ptAA4y
            self.AA6 = np.array([ptAA6x, ptAA6y])

            ptAA7x = ptAA1x
            ptAA7y = ptAA1y + self.data_object.flange_thickness_T2
            self.AA7 = np.array([ptAA7x, ptAA7y])

            ptAA8x = ptAA7x + self.data_object.column_width_B1 / 2 - self.data_object.plate_thickness_p1 - self.data_object.web_thickness_tw1 / 2
            ptAA8y = ptAA7y
            self.AA8 = np.array([ptAA8x, ptAA8y])

            ptAA9x = ptAA7x + self.data_object.beam_length_L2
            ptAA9y = ptAA7y
            self.AA9 = np.array([ptAA9x, ptAA9y])

            ptAA10x = ptAA4x
            ptAA10y = ptAA4y - self.data_object.flange_thickness_T2
            self.AA10 = np.array([ptAA10x, ptAA10y])

            ptAA11x = ptAA10x - self.data_object.beam_length_L2 + self.data_object.column_width_B1 / 2 - self.data_object.plate_thickness_p1 - self.data_object.web_thickness_tw1 / 2
            ptAA11y = ptAA10y
            self.AA11 = np.array([ptAA11x, ptAA11y])

            ptAA12x = ptAA10x - self.data_object.beam_length_L2
            ptAA12y = ptAA10y
            ...
            ptP1x = ptA1x + self.data_object.column_width_B1/2 + self.data_object.web_thickness_tw1/2
        ...
        ptS1x = ptAA1x - self.data_object.plate_thickness_p1 - self.data_object.column_width_B1/2- self.data_object.web_thickness_tw1/2
        ptS1y = ptAA1y
        self.S1 = np.array([ptS1x, ptS1y])

        ptS2x = ptAA1x - self.data_object.plate_thickness_p1 - self.data_object.web_thickness_tw1
        ptS2y = ptAA1y
        self.S2 = np.array([ptS2x, ptS2y])

        ptS4x = ptAA7x - self.data_object.plate_thickness_p1 - self.data_object.column_width_B1/2 - self.data_object.web_thickness_tw1/2
        ptS4y = ptAA7y
        self.S4 = np.array([ptS4x, ptS4y])

        ptS3x = ptAA7x - self.data_object.plate_thickness_p1 - self.data_object.web_thickness_tw1
        ptS3y = ptAA7y
        self.S3 = np.array([ptS3x, ptS3y])

        ptS5x = ptAA12x - self.data_object.plate_thickness_p1 - self.data_object.column_width_B1 / 2 - self.data_object.web_thickness_tw1/2
        ptS5y = ptAA12y
        self.S5 = np.array([ptS5x, ptS5y])

        ptS6x = ptAA12x - self.data_object.plate_thickness_p1 - self.data_object.web_thickness_tw1
        ptS6y = ptAA12y
        self.S6 = np.array([ptS6x, ptS6y])

        ptS7x = ptAA6x - self.data_object.plate_thickness_p1 - self.data_object.flange_thickness_T1
        ptS7y = ptAA6y
        self.S7 = np.array([ptS7x, ptS7y])

        ptS8x = ptAA6x - self.data_object.plate_thickness_p1 - self.data_object.column_width_B1 / 2 - self.data_object.web_thickness_tw1/2
        ptS8y = ptAA6y
        self.S8 = np.array([ptS8x, ptS8y])


        if self.data_object.input_dict != "Flush end plate":
            # ================ Stiffener - UP ==================
            ...
            ptSU7x = ptAA1x + self.data_object.column_width_B1/2 - self.data_object.plate_thickness_p1 - self.data_object.web_thickness_tw1/2
            ptSU7y = ptAA1y - self.data_object.stiffener_height
            self.SU7 = np.array([ptSU7x, ptSU7y])
        ...
        if self.data_object.input_dict == "Extended both ways":
            ...
            ptSD1x = ptAA6x
            ptSD1y = ptAA6y + self.data_object.stiffener_height
            ...
            ptSD6x = ptAA6x
            ptSD6y = ptAA6y + self.data_object.stiffener_NotchBottom
            self.SD6 = np.array([ptSD6x, ptSD6y])

            ptSD5x = ptAA6x + self.data_object.stiffener_NotchBottom
            ptSD5y = ptAA6y
            self.SD5 = np.array([ptSD5x, ptSD5y])

            ptSD4x = ptAA6x + self.data_object.stiffener_length
            ptSD4y = ptAA6y
            ...
            ptSD7x = ptAA6x + self.data_object.column_width_B1 / 2 - self.data_object.plate_thickness_p1 - self.data_object.web_thickness_tw1 / 2
            ptSD7y = ptAA6y + self.data_object.stiffener_height
    ...
    def call_front(self, filename):
        ...
        dwg.add(dwg.polyline(points=[self.A1, self.A2, self.A3, self.A4, self.A1], stroke='black', fill='none',
                             stroke_width=2.5))
        dwg.add(dwg.line(self.A5, self.A8).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
        dwg.add(dwg.line(self.A6, self.P1).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
        ...
        dwg.add(dwg.line(self.AA8, self.AA9).stroke('black', width=2.5, linecap='square'))
        dwg.add(dwg.line(self.AA11, self.AA10).stroke('black', width=2.5, linecap='square'))
        dwg.add(dwg.line(self.AA7, self.AA8).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
        dwg.add(dwg.line(self.AA12, self.AA11).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
        dwg.add(dwg.line(self.AA6, self.AA5).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
        dwg.add(dwg.line(self.S1, self.S2).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
        dwg.add(dwg.line(self.S3, self.S4).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
        dwg.add(dwg.line(self.S5, self.S6).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
        dwg.add(dwg.line(self.S7, self.S8).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))

        if self.data_object.no_of_bolts == 20 and self.data_object.input_dict == "Extended both ways":
            dwg.add(dwg.line(self.SU1, self.SU7).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
            dwg.add(dwg.line(self.SU7, self.SU2).stroke('black', width=2.5, linecap='square'))
            dwg.add(dwg.line(self.SU2, self.SU3).stroke('black', width=2.5, linecap='square'))
            dwg.add(dwg.line(self.SU3, self.SU4).stroke('black', width=2.5, linecap='square'))
            dwg.add(dwg.line(self.SU5, self.SU6).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
            dwg.add(dwg.line(self.SD1, self.SD7).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
            dwg.add(dwg.line(self.SD7, self.SD2).stroke('black', width=2.5, linecap='square'))
            dwg.add(dwg.line(self.SD2, self.SD3).stroke('black', width=2.5, linecap='square'))
            dwg.add(dwg.line(self.SD3, self.SD4).stroke('black', width=2.5, linecap='square'))
            dwg.add(dwg.line(self.SD5, self.SD6).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
        elif self.data_object.no_of_bolts == 12 and self.data_object.input_dict == "Extended one way":
            dwg.add(
                dwg.line(self.SU1, self.SU7).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
            dwg.add(dwg.line(self.SU7, self.SU2).stroke('black', width=2.5, linecap='square'))
            dwg.add(dwg.line(self.SU2, self.SU3).stroke('black', width=2.5, linecap='square'))
            dwg.add(dwg.line(self.SU3, self.SU4).stroke('black', width=2.5, linecap='square'))
            dwg.add(
                dwg.line(self.SU5, self.SU6).stroke('black', width=2.5, linecap='square').dasharray(dasharray=[5, 5]))
        ...
        if self.data_object.input_dict == "Extended both ways":
            ...
            for i in range(bitfr):
                if self.data_object.no_of_bolts == 4:
                    ptx = self.AA1 - (self.data_object.flange_thickness_T2 + self.Lv) * np.array([0, -1]) - (
                                self.data_object.plate_thickness_p1 + self.data_object.web_thickness_tw1) * np.array(
                        [1, 0])
            ...
            for i in range(bibfr):
                ...
                rect_length = float(self.data_object.plate_thickness_p1 + self.data_object.web_thickness_tw1)
                dwg.add(
                    dwg.rect(insert=ptx1, size=(rect_length, rect_width), fill='black', stroke='black',
                             stroke_width=2.5))
        ...
        # ------------------------------------------  Primary Beam 1& 2 -------------------------------------------
        point = self.A1 + self.data_object.column_width_B1/2 * np.array([1, 0])
        theta = 60
        offset = 50
        ...
        point = self.AA3 - 100 * np.array([1, 0])
        theta = 60
        offset = 50
        ...
        # ------------------------------------------  View details-------------------------------------------
        ptx = self.A3 - 100 * np.array([1, 0]) + 300 * np.array([0, 1])
...
class BC_WW_2DTop(object):
    ...
    def __init__(self, extnd_common_object):
        ...
        ptA4x = ptA1x
        ptA4y = ptA1y + self.data_object.flange_thickness_T1
        self.A4 = np.array([ptA4x, ptA4y])

        ptA5x = ptA1x
        ptA5y = ptA1y + self.data_object.column_depth_D1 - self.data_object.flange_thickness_T1
        self.A5 = np.array([ptA5x, ptA5y])

        ptA6x = ptA5x + self.data_object.column_width_B1
        ptA6y = ptA5y
        self.A6 = np.array([ptA6x, ptA6y])

        ptA7x = ptA6x
        ptA7y = ptA6y + self.data_object.flange_thickness_T1
        self.A7 = np.array([ptA7x, ptA7y])

        ptA8x = ptA5x
        ptA8y = ptA5y + self.data_object.flange_thickness_T1
        self.A8 = np.array([ptA8x, ptA8y])

        ptA9x = ptA4x + self.data_object.column_width_B1/2 - self.data_object.web_thickness_tw1/2
        ptA9y = ptA4y
        self.A9 = np.array([ptA9x, ptA9y])

        ptA10x = ptA9x + self.data_object.web_thickness_tw1
        ptA10y = ptA9y
        self.A10 = np.array([ptA10x, ptA10y])

        ptA11x = ptA6x - self.data_object.column_width_B1/2 + self.data_object.web_thickness_tw1/2
        ptA11y = ptA6y
        self.A11 = np.array([ptA11x, ptA11y])

        ptA12x = ptA11x - self.data_object.web_thickness_tw1
        ptA12y = ptA11y
    ...
    def call_top(self, filename):
        ...
        if self.data_object.no_of_bolts == 20 and self.data_object.input_dict == "Extended both ways":
            ...
            pattern = dwg.defs.add(dwg.pattern(id="diagonalHatch", size=(6, 6), patternUnits="userSpaceOnUse",
                                               patternTransform="rotate(45 2 2)"))
            pattern.add(dwg.path(d="M 0,1 l 6,0", stroke='#000000', stroke_width=2.5))
            dwg.add(dwg.rect(insert=self.AA5  - self.data_object.weld_plate_thickness_p2 * np.array([0,1]), size=(self.data_object.stiffener_length,
                self.data_object.weld_plate_thickness_p2), fill="url(#diagonalHatch)", stroke='white', stroke_width=1.0))
            pattern.add(dwg.path(d="M 0,1 l 6,0", stroke='#000000', stroke_width=2.5))
            dwg.add(dwg.rect(insert=self.AA8,size=(self.data_object.stiffener_length,
                     self.data_object.weld_plate_thickness_p2), fill="url(#diagonalHatch)",stroke='white', stroke_width=1.0))
            ...
            pattern = dwg.defs.add(dwg.pattern(id="diagonalHatch", size=(6, 6), patternUnits="userSpaceOnUse",
                                               patternTransform="rotate(45 2 2)"))
            pattern.add(dwg.path(d="M 0,1 l 6,0", stroke='#000000', stroke_width=2.5))
            dwg.add(dwg.rect(insert=self.AA5 - self.data_object.weld_plate_thickness_p2 * np.array([0, 1]),
                             size=(self.data_object.stiffener_length,
                                   self.data_object.weld_plate_thickness_p2), fill="url(#diagonalHatch)",
        ...
        # ------------------------------------------  Sectional arrow -------------------------------------------
        pt_a1 = self.A8 - (200) * np.array([0, -1]) - (100 * np.array([1, 0]))
        pt_b1 = pt_a1 + (50 * np.array([0, -1]))
        ...
        pt_a4 = pt_a3 + (self.data_object.column_depth_D1 * np.array([0, 1]))
...
class BC_WW_2DSide(object):
    ...
    def __init__(self, extnd_common_object):
        ...
        ptAA1x = ptP1x + self.data_object.plate_width_B1 / 2 - self.data_object.column_depth_D1 / 2
        ptAA1y = ptP1y + self.data_object.plate_length_L1 / 2 - self.data_object.column_length_L1 / 2
        self.AA1 = np.array([ptAA1x, ptAA1y])

        ptAA2x = ptAA1x + self.data_object.column_depth_D1
        ptAA2y = ptAA1y
        self.AA2 = np.array([ptAA2x, ptAA2y])

        ptAA3x = ptAA2x
        ptAA3y = ptAA2y + self.data_object.column_length_L1
        self.AA3 = np.array([ptAA3x, ptAA3y])

        ptAA4x = ptAA1x
        ptAA4y = ptAA1y + self.data_object.column_length_L1
        self.AA4 = np.array([ptAA4x, ptAA4y])

        ptAA5x = ptAA1x + self.data_object.flange_thickness_T1
        ptAA5y = ptAA1y
        self.AA5 = np.array([ptAA5x, ptAA5y])

        ptAA6x = ptAA2x - self.data_object.flange_thickness_T1
        ptAA6y = ptAA2y
        self.AA6 = np.array([ptAA6x, ptAA6y])

        ptAA7x = ptAA3x - self.data_object.flange_thickness_T1
        ptAA7y = ptAA3y
        self.AA7 = np.array([ptAA7x, ptAA7y])

        ptAA8x = ptAA4x + self.data_object.flange_thickness_T1
        ptAA8y = ptAA4y
        self.AA8 = np.array([ptAA8x, ptAA8y])
    ...
    def call_side(self, filename):
        ...
        ht = (int(self.data_object.column_length_L1 + 400 + 200))
        dwg = svgwrite.Drawing(filename, size=('100%', '100%'), viewBox=(
        ...
        if self.data_object.no_of_bolts == 20 and self.data_object.input_dict == "Extended both ways":
            ...
            dwg.add(dwg.rect(insert=self.P5  - self.data_object.stiffener_weldsize * np.array([1, 0]),
                             size=(self.data_object.stiffener_weldsize, (self.data_object.stiffener_height)),
                             fill="url(#diagonalHatch)", stroke='white', stroke_width=1.0))
            pattern.add(dwg.path(d="M 0,1 l 6,0", stroke='#000000', stroke_width=2.5))
            dwg.add(dwg.rect(insert=self.P6,
                             size=(self.data_object.stiffener_weldsize, (self.data_object.stiffener_height)),
                             fill="url(#diagonalHatch)", stroke='white', stroke_width=1.0))
            pattern.add(dwg.path(d="M 0,1 l 6,0", stroke='#000000', stroke_width=2.5))
            dwg.add(dwg.rect(insert=self.SD1 - self.data_object.stiffener_weldsize * np.array([1, 0]),
                             size=(self.data_object.stiffener_weldsize, (self.data_object.stiffener_height)),
                             fill="url(#diagonalHatch)", stroke='white', stroke_width=1.0))
            pattern.add(dwg.path(d="M 0,1 l 6,0", stroke='#000000', stroke_width=2.5))
            dwg.add(dwg.rect(insert=self.SD2,
                             size=(self.data_object.stiffener_weldsize, (self.data_object.stiffener_height)),
                             fill="url(#diagonalHatch)", stroke='white', stroke_width=1.0))
        ...
        if self.data_object.input_dict == "Extended both ways":
                    ...
                    if self.data_object.no_of_bolts == 8:
                        ...
                        pt = self.P4 + self.data_object.end_dist * \
                             np.array([0, -1]) + self.data_object.edge_dist * np.array([1, 0]) + (
                                         i - 1) * self.data_object.pitch910 * np.array([0, -1]) + \
                             (j - 1) * self.data_object.cross_centre_gauge_dist * np.array([1, 0])
                    dwg.add(dwg.circle(center=pt, r=bolt_r, stroke='black', fill='none', stroke_width=1.5))
                    pt_C = pt - (bolt_r + 4) * np.array([1, 0])
                    pt_D = pt + (bolt_r + 4) * np.array([1, 0])
            ...
            for i in range(1, (bibfr + 1)):
                ...
                for j in range(1, (nofc + 1)):
                    if self.data_object.no_of_bolts == 8:
                        ...
                        pt = self.P1 + ((self.data_object.plate_length_L1 + self.data_object.beam_depth_D2) / 2 - self.data_object.flange_thickness_T1 - self.data_object.Lv) * np.array(
                            [0, 1]) + self.data_object.edge_dist * np.array([1, 0]) + (i - 1) * self.data_object.pitch56 * np.array([0, -1]) + (j - 1) * self.data_object.cross_centre_gauge_dist * np.array([1, 0])
                    elif self.data_object.no_of_bolts == 20:
                        pt = self.P1 + ((self.data_object.plate_length_L1 + self.data_object.beam_depth_D2) / 2 - self.data_object.flange_thickness_T1 - self.data_object.Lv) * np.array(
                            [0, 1]) + self.data_object.edge_dist * np.array([1, 0]) + (i - 1) * self.data_object.pitch67 * np.array([0, -1]) + (j - 1) * self.data_object.cross_centre_gauge_dist * np.array([1, 0])
            ...
            for i in range(1, (bitfr + 1)):
                ...
                for j in range(1, (nofc + 1)):
                    if self.data_object.no_of_bolts == 6:
                        pt = self.P1 + (
                                    self.data_object.plate_length_L1 - self.data_object.flange_projection - self.data_object.beam_depth_D2 + self.data_object.flange_thickness_T2 + self.data_object.Lv) * np.array(
                            [0, 1]) + self.data_object.edge_dist * np.array([1, 0]) + (
                                         i - 1) * self.data_object.pitch23 * np.array(
                            [0, 1]) + (j - 1) * self.data_object.cross_centre_gauge_dist * np.array([1, 0])
                    elif self.data_object.no_of_bolts == 8:
                        pt = self.P1 + (
                                    self.data_object.plate_length_L1 - self.data_object.flange_projection - self.data_object.beam_depth_D2 + self.data_object.flange_thickness_T2 + self.data_object.Lv) * np.array(
            ...
            for i in range(1, (bibfr + 1)):
                ...
                for j in range(1, (nofc + 1)):
                    pt = self.P4 + (
                                self.data_object.flange_projection + self.data_object.flange_thickness_T2 + self.data_object.Lv) * np.array(
                        [0, -1]) + self.data_object.edge_dist * np.array([1, 0]) + (
                                     i - 1) * self.data_object.pitch23 * np.array([0, -1]) + (
                                     j - 1) * self.data_object.cross_centre_gauge_dist * np.array([1, 0])
        ...
        if self.data_object.input_dict == "Flush end plate":
            ...
            self.data_object.draw_dimension_outer_arrow(dwg, point1, point2,
                                                        str(self.data_object.cross_centre_gauge_dist),
                                                        params)
            ...
            if self.data_object.no_of_bolts == 20 and self.data_object.input_dict == "Extended both ways":
                ...
                ptxx3 = np.array(pt_outside_top_column_list[0][1])
                ptyy3 = ptxx3 + (self.data_object.beam_width_B2 + 50) * np.array([1, 0])
                self.data_object.draw_faint_line(ptxx3, ptyy3, dwg)

                point3 = np.array(pt_outside_top_column_list[1][1])
                ptyy3 = ptxx3 + (self.data_object.beam_width_B2 + 50) * np.array([1, 0])
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "left",
                          "endlinedim": 10,
                          "arrowlen": 20}
                self.data_object.draw_dimension_outer_arrow(dwg, ptxx2, point3, str(self.data_object.pitch12), params)
            else:
                point2 = ptxx2 + self.data_object.Lv * np.array([0, 1])
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "left",
                          "endlinedim": 10, "arrowlen": 20}
        ...
        if self.data_object.input_dict == "Extended both ways":
            ...
            if self.data_object.no_of_bolts == 8:
                ...
                point3 = ptx2 + self.data_object.Lv * np.array([0, 1])
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "right",
                          "endlinedim": 10, "arrowlen": 20}
                self.data_object.draw_dimension_outer_arrow(dwg, point3, ptx2, str(self.data_object.Lv), params)
                ...
                point6 = ptx4 + self.data_object.Lv * np.array([0, -1])
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "right",
                          "endlinedim": 10,
                          "arrowlen": 20}
                self.data_object.draw_dimension_outer_arrow(dwg, ptx4, point6, str(self.data_object.Lv), params)
            ...
            if self.data_object.no_of_bolts == 8:
                ...
                point3 = ptx7 + self.data_object.Lv * np.array([0, 1])
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "right",
                          "endlinedim": 10, "arrowlen": 20}
                ...
                point3 = ptx8 + self.data_object.Lv * np.array([0, 1])
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "right",
                          "endlinedim": 10, "arrowlen": 20}
                self.data_object.draw_dimension_outer_arrow(dwg, point3, ptx8, str(self.data_object.Lv), params)
            ...
            if self.data_object.no_of_bolts == 20:
                ...
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "left",
                          "endlinedim": 10, "arrowlen": 20}
                self.data_object.draw_dimension_outer_arrow(dwg, point2, ptx3, str(self.data_object.Lv), params)
            else:
                point2 = ptx2 + self.data_object.Lv * np.array([0, -1])
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "left",
                          "endlinedim": 10, "arrowlen": 20}
            ...
            if self.data_object.no_of_bolts == 6:
                ...
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "left",
                          "endlinedim": 10, "arrowlen": 20}
                self.data_object.draw_dimension_outer_arrow(dwg, point2, ptx1, str(self.data_object.Lv), params)
                ...
                point2 = np.array(pt_inside_bottom_column_list[0][1])
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "left",
                          "endlinedim": 10, "arrowlen": 20}
                self.data_object.draw_dimension_outer_arrow(dwg, ptx2, point2, str(self.data_object.pitch34), params)
                ...
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "right",
                          "endlinedim": 10, "arrowlen": 20}
                self.data_object.draw_dimension_outer_arrow(dwg, ptx2, ptx3, str(self.data_object.pitch23), params)
                ...
                point4 = np.array(pt_inside_bottom_column_list[0][1])
                point7 = point4 + self.data_object.pitch56 * np.array([0, -1])
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "left",
                          "endlinedim": 10, "arrowlen": 20}
                self.data_object.draw_dimension_outer_arrow(dwg, point7, point4, str(self.data_object.pitch56), params)
            ...
            if self.data_object.no_of_bolts == 6:
                ...
                point1 = ptx2 + self.data_object.Lv * np.array([0, 1])
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "right",
                          "endlinedim": 10, "arrowlen": 20}
                self.data_object.draw_dimension_outer_arrow(dwg, point1, ptx2, str(self.data_object.Lv), params)
                ...
                point2 = ptx5 + self.data_object.Lv * np.array([0, 1])
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "left",
                          "endlinedim": 10, "arrowlen": 20}
            ...
            if self.data_object.no_of_bolts == 4:
                ...
                point5= point2 + self.data_object.pitch12 * np.array([0, -1])
                point5y =  point5 + (self.data_object.beam_width_B2 + 50) * np.array([1, 0])
                self.data_object.draw_faint_line( point5, point5y, dwg)
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "left",
                          "endlinedim": 10, "arrowlen": 20}
                self.data_object.draw_dimension_outer_arrow(dwg, point2, point5, str(self.data_object.pitch12), params)

                point1 = np.array(pt_inside_bottom_column_list[1][1])
                point1y = point1 + (self.data_object.beam_width_B2 + 50) * np.array([1, 0])
                self.data_object.draw_faint_line(point1, point1y, dwg)
                point4 = np.array(pt_inside_top_column_list[1][1])
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "left",
                          "endlinedim": 10, "arrowlen": 20}
                self.data_object.draw_dimension_outer_arrow(dwg, point4, point1, str(self.data_object.pitch23), params)
                ...
                pty4 = ptx4 + (self.data_object.beam_width_B2 + 50) * np.array([1, 0])
                self.data_object.draw_faint_line(ptx4, pty4, dwg)
                point2 = ptx4 + (self.data_object.pitch12) * np.array([0, 1])
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "left",
                          "endlinedim": 10, "arrowlen": 20}
                self.data_object.draw_dimension_outer_arrow(dwg, ptx4, point2, str(self.data_object.pitch12), params)
                ...
                pty5 = ptx5 + (self.data_object.beam_width_B2 + 50) * np.array([1, 0])
                self.data_object.draw_faint_line(ptx5, pty5, dwg)
                point3 = ptx5 + (self.data_object.pitch23) * np.array([0, -1])
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "right",
                          "endlinedim": 10, "arrowlen": 20}
                self.data_object.draw_dimension_outer_arrow(dwg, ptx5, point3, str(self.data_object.pitch23), params)

                point4 = np.array(pt_inside_bottom_column_list[2][1])
                point7 = np.array(pt_inside_top_column_list[2][1])
                params = {"offset": (self.data_object.beam_width_B2 + 50), "textoffset": 10, "lineori": "left",
                          "endlinedim": 10, "arrowlen": 20}
                self.data_object.draw_dimension_outer_arrow(dwg, point7, point4, str(self.data_object.pitch34), params)
        ...
        # ------------------------------------------  End Plate 1 -------------------------------------------
        point = self.P1 + (self.data_object.plate_width_B1 / 2 + self.data_object.plate_width_B1 / 4) * np.array([1, 0])
        theta = 60
        offset = 50
        textup = "End plate " + str(self.data_object.plate_length_L1) + "x" + str(
            self.data_object.plate_width_B1) + "x" + str(
