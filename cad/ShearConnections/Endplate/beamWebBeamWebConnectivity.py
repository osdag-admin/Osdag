...
class BeamWebBeamWeb(object):
    ...
    def create_plate_geometry(self):
        plate_origin = self.beam.sec_origin + (self.plate.W / 2) * (-self.beam.uDir) + (self.plate.T / 2) * (
            -self.beam.wDir) + (self.beam.D / 2 - self.notch.height - self.plate.L / 2) * (
                           self.beam.vDir)  # (self.beam.D/2 - self.notch.height - self.plate.L/2) * (-self.beam.vDir) + (self.plate.W/2) * (self.beam.uDir) + (self.plate.T/2) * (-self.beam.wDir)
